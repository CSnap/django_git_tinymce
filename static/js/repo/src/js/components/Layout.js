          ...
          <div class="panel panel-success">
            ...
            <table class="table">
              ...
              {this.props.files.files.map((file) => {
                ...
                return <tr key={file.id}><th scope="row">{icon} {fileLink} &nbsp;{editLink}</th><td>{ files.is_owner && <a href={`/${window.props.repo_owner}/${window.props.repo_name}/${(window.props.directory !== '') ? `${window.props.directory}/` : ``}blob/${file.name}/delete`}><font style={{fontSize: '.75em', color: '#f33'}}>delete</font></a>}</td><td><a href={`commit/${file.id}`}>{file.id}</a></td></tr>
              })}
              </tbody>
            </table>
          </div>
